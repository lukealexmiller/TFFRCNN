...
#   gt_cls_names: the class names corresponding to gt_boxes
    ...
    assert( gt_boxes.shape[0] + len(hardneg_inds) > 0 )
    ...
    for i in range( gt_boxes.shape[0] ):
        ...
        xml_f.write( '    <object>\n')
        xml_f.write( '        <name>%s</name>\n'% gt_cls_names[i] )
        xml_f.write( '        <difficult>%s</difficult>\n'% str(0) )
        xml_f.write( '        <bndbox>\n')
        xml_f.write( '            <xmin>%s</xmin>\n'% str(x1+1) )
        xml_f.write( '            <ymin>%s</ymin>\n'% str(y1+1) )
        xml_f.write( '            <xmax>%s</xmax>\n'% str(x2+1) )
        xml_f.write( '            <ymax>%s</ymax>\n'% str(y2+1) )
        xml_f.write( '        </bndbox>\n')
        xml_f.write( '    </object>\n')

    for i in range( len(hardneg_inds) ):
        x1 = hardnegs[i,0]
        y1 = hardnegs[i,1]
        x2 = hardnegs[i,2]
        y2 = hardnegs[i,3]
        ...
        xml_f.write( '    <object>\n')
        xml_f.write( '        <name>__background__</name>\n')
        xml_f.write( '        <difficult>%s</difficult>\n'% str(0) )
        xml_f.write( '        <bndbox>\n')
        xml_f.write( '            <xmin>%s</xmin>\n'% str(x1+1) )
        xml_f.write( '            <ymin>%s</ymin>\n'% str(y1+1) )
...
#   visualize both the hardnegs (in red) and groundtruth fg&bg bbox (in green)
    ...
    plt.cla()
    plt.imshow(im)
    for i in xrange( hardnegs.shape[0] ):
        bbox = hardnegs[i, :4]
        plt.gca().add_patch(
            plt.Rectangle((bbox[0], bbox[1]),
                          bbox[2] - bbox[0],
...
#                vis_detections(image, imdb.classes[j], cls_dets)
        ...
        detections_im_i = np.empty( (0,5), dtype=np.float32)
        for j in xrange(1, imdb.num_classes):
            if len( all_boxes[j][i] )==0:
               continue
            detections_im_i = np.vstack( (detections_im_i, all_boxes[j][i]) )
